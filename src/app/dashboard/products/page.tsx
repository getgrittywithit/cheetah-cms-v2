...
export default function ProductsPage() {
  ...
  const syncProducts = async (brandSlug?: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(`Synced ${data.synced_count || data.count || 0} products from Printful for ${brand}`)
      } else {
  ...
  const syncSingleProduct = async (productId: number) => {
    ...
    try {
      const response = await fetch('/api/printful/sync', {
        ...
        body: JSON.stringify({ product_id: productId, brand: selectedBrand })
      })
  ...
  const publishToShopify = async () => {
    ...
    try {
      ...
      if (data.success) {
        alert(`Successfully published "${data.shopify_product?.title || 'product'}" to ${selectedBrand}!`)
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Sync and manage your Printful products with AI content generation and canvas maker
        </p>
      </div>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full m-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold">Publish Product</h2>
                <button
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between pt-4 border-t">
                  ...
                  >
                    <ShoppingCart className="w-4 h-4 mr-2" />
                    {publishing === showPublishForm.id ? 'Publishing...' : 'Publish Product'}
                  </button>
                </div>
              </div>
            </div>
