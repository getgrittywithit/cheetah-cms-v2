...
import { RefreshCw, Package, ExternalLink, ShoppingCart, Eye, Wand2, Plus } from 'lucide-react'
...
export default function BrandProductsPage({ params }: { params: { brand: string } }) {
  ...
  const syncProducts = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(`Successfully synced ${data.synced_count || data.count || 0} products from Printful to ${brandConfig?.name}!`)
  ...
  const syncSingleProduct = async (productId: number) => {
    ...
    try {
      const response = await fetch('/api/printful/sync', {
        ...
        body: JSON.stringify({ product_id: productId, brand: params.brand })
      })
      ...
      if (data.success) {
        ...
        setPrintfulProducts(prev => prev.map(p => 
          p.id === productId ? data.product : p
        ))
        fetchProducts() // Refresh current products
      } else {
        alert(`Sync failed: ${data.error}`)
      }
    } catch (error) {
  ...
  const publishToShopify = async () => {
    ...
    try {
      ...
      if (data.success) {
        alert(`Successfully published "${data.shopify_product.title}" to ${brandConfig?.name}!`)
  ...
  const prepareForShopify = (product: PrintfulProduct) => {
    ...
    setPublishForm({
      ...
      tags: `canvas art, wall decor, motivational, home decor, ${params.brand}`,
  ...
  useEffect(() => {
    fetchProducts()
    // Initialize with some Printful products if available
    syncProducts()
  }, [params.brand])
    ...
    <div className="p-6">
        ...
        <nav className="-mb-px flex space-x-8">
          ...
          >
            <div className="flex items-center space-x-2">
              ...
              <span>Printful Sync ({printfulProducts.length})</span>
            </div>
          </button>
        </nav>
      </div>
      ...
      {activeTab === 'printful' && (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-semibold text-gray-900 mb-2">
                  Printful Products for {brandConfig.name}
                </h2>
                <p className="text-gray-600">
                  Sync completed products from Printful to {brandConfig.name}'s catalog
                </p>
              </div>
              <button
          ...
          {printfulProducts.length === 0 && !loading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-semibold">Products ({printfulProducts.length})</h3>
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              <AIProductGenerator
                ...
                onGenerate={handleAIDataGenerated}
